 ...
 *
 */
class Tx_HypeOptimum_Utility_Concatenator_StyleConcatenator
	...
	public function concatenate() {
		...
		# determine new path
		$newPath = 'typo3temp/' . $hash . '.css';

		# create compressed file
		if(!file_exists($this->getBasePath() . $newPath)) {

			# get and prepare file contents
			$data = '';
			foreach($this->files as $file => $options) {
				$data .= '@media ' . $options['media'] . ' {' . file_get_contents(realpath(PATH_site . $file)) . '}';
			}

			# sniff out font face declarations and stick 'em on top
			preg_match_all('~\@font-face.?\{.*\}~isU', $data, $matches);
			...
			# write new file
			if($data) {
				t3lib_div::writeFileToTypo3tempDir($this->getBasePath() . $newPath, $data);
				unset($data);
			}
		}
	}
